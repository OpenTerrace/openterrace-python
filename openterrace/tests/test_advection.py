...
def test_advection_block():
    ot.fluid = ot.Phase(n=500, type='fluid')
    ot.fluid.select_substance_on_the_fly(rho=1000, cp=1, k=1)
    ot.fluid.select_domain_shape(domain='block_1d', A=1, L=1)
    ot.fluid.select_schemes(diff='central_difference_1d', conv='upwind_1d')
    ot.fluid.select_initial_conditions(T=273.15+100, mdot=1)
    ot.fluid.select_bc(bc_type='dirichlet', parameter='T', position=(slice(None, None, None), 0), value=273.15+200)
    ot.fluid.select_bc(bc_type='neumann', parameter='T', position=(slice(None, None, None), -1))
    ot.fluid.select_output(times=range(0, 900+15, 60), parameters=['T'])
