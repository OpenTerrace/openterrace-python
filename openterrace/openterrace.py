...
class GlobalParameters:
    ...
    class Phase:
        ...
        def select_substance_on_the_fly(self, cp:float=None, rho:float=None, k:float=None, mu:float=None):
        ...
        def select_porosity(self, phi=1):
        ...
        def select_initial_conditions(self, T=None, mdot=None):
            ...
            for bc in self.bcs:
                ...
                if bc['type'] == 'neumann':
                    if bc['position'] == np.s_[:,0]:
                        self.h[bc['position']] = self.h[bc['position']] + (2*self.T[:,1]*self.D[0,:,1] - 2*self.T[:,0]*self.D[1,:,0] - self.F[0,:,1]*self.T[:,1] + self.F[1,:,0]*self.T[:,0]) / (self.rho[:,0]*self.domain.V[0])*dt
                    if bc['position'] == np.s_[:,-1]:
                        self.h[bc['position']] = self.h[bc['position']] + (2*self.T[:,-2]*self.D[1,:,-2] - 2*self.T[:,-1]*self.D[0,:,-1] + self.F[1,:,-2]*self.T[:,-2] - self.F[0,:,-1]*self.T[:,-1]) / (self.rho[:,-1]*self.domain.V[-1])*dt
